...
def list_media(db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> list[MediaAssetOut]:
    assets = db.query(MediaAsset).order_by(MediaAsset.created_at.desc()).all()
    return [_to_out(asset) for asset in assets]


@router.delete("/{asset_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_media(asset_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> None:
